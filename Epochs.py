...
def getEpochs4sec(epochs,onset_4s,fs,epoch_duration=4):
    ...
    data = {}
    for keys in epochs.keys():
        d = {'Time': epochs[keys]['Time'], 'ECG': epochs[keys]['ECG'], 'EDA': epochs[keys]['EDA'],
             'EDA_Clean': epochs[keys]['EDA_Clean'],'EDA_Tonic': epochs[keys]['EDA_Tonic'],
             'EDA_Phasic': epochs[keys]['EDA_Phasic']}
        df = pd.DataFrame(data=d)
        data[keys] = nk.epochs_create(df, events=onset_4s[keys], sampling_rate=fs, epochs_start=0, epochs_end=epoch_duration)
