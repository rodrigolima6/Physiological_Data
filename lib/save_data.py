...
for i, folder in enumerate(folders):
    ...
    timestamps, intialLabels = acquisition.getPsychoTimestamps(), acquisition.getPsychoActivity()
    
    acquisition.deleteArtifacts()
    acquisition.getBiosignalsSensors(ALL, rightPos=True)
    acquisition.convertSensors()
    acquisition.segmentWindowingBiosignals(acquisition.signal, timestamps, intialLabels, timeWindow=.128)
    ...
    if len(segments) == 0:
        ...
        segments = np.vstack([segments, concatenateSegments(acquisition.segmentedBiosignals)])
        labels = np.concatenate([labels, acquisition.labels])
        group = np.concatenate([group, [folder] * acquisition.segmentedBiosignals.shape[0]])
