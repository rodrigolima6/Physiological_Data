...
class Latent:
    ...
    def interpolateInterfaces(self, interface):
        ...
        time = np.linspace(
            time_DF[0], time_DF[-1], int(np.round((time_DF[-1] - time_DF[0]) * self.fs))
        )
        ...
        data = self.fillValues(
            self.data[interface],
            interface,
            data,
            time_DF,
            keys["fill"] + keys["interpolate"],
            columns["fill"] + columns["interpolate"],
        )
        data = self.interpolateValues(
            self.data[interface],
            interface,
            data,
            time_DF,
    ...
    def duplicateValues(self, duplicate_data, interface, new_data, data, keys, columns):
        ...
        for i in range(len(time)):
            ...
            try:
                while np.abs(data[duplicate_index + 1] - time[i]) < np.abs(
                    data[duplicate_index] - time[i]
                ):
                    duplicate_index += 1
            except IndexError:
    ...
    def fillValues(self, fill_data, interface, new_data, data, keys, columns):
        ...
        for i, value in enumerate(data):
            ...
            try:
                while np.abs(time[fill_index + 1] - value) < np.abs(
                    time[fill_index] - value
                ):
                    fill_index += 1
            except IndexError:
    ...
    def interpolateValues(
        self, interpolate_data, interface, new_data, data, keys, columns
    ):
        ...
        for j, key in enumerate(columns[1:]):
            for i in range(1, len(new_data)):
                print(
                    f"Interpolating {interface} Data: {i*100/len(new_data):.2f}%",
                    end="\r",
                )
    ...
    def identifyTypeInterp(self, interface):
        if interface.lower() == "mouse":
            ...
            return {
                ...
                "interpolate": ["time", "event_timestamp"],
                "duplicate": ["type", "tab_id"],
                "fill": [
                    "key_code",
                    "shift_key",
                    "alt_key",
