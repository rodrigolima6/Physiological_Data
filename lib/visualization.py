...
parser = argparse.ArgumentParser(
    description="Identify the path to the folder that contains all signals, photos and PsychoPy files."
)
parser.add_argument("-P", "--path")
args = parser.parse_args()

if args.path:
    folder = rf"D:\Google Drive\Faculdade\Doutoramento\Acquisitions\new_biosignalsnotebooks\acquisitions\Acquisitions\{args.path}"
else:
    raise Exception(
        "Please specify the path to the folder containing all signals, photos and PsychoPy files."
    )
