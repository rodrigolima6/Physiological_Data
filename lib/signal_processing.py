...
def bsnbSmooth(input_signal, window_len=10, window="hanning"):
    sig = np.r_[
        input_signal[window_len:0:-1],
        input_signal,
        input_signal[-2 : -window_len - 2 : -1],
    ]
        ...
        if prev_value > 0 and diff_data[i] < 0 and data[i] > y_threshold or i == 0:
            if x_threshold > 0:
                if i + x_threshold < len(diff_data):
                    pos, peak = higherPeak(
                        getValuesArray(data, i, i + x_threshold),
                        getValuesArray(diff_data, i, i + x_threshold),
                        y_threshold,
                    )

                else:
                    pos, peak = higherPeak(
                        getValuesArray(data, i, len(diff_data) - 1),
                        getValuesArray(diff_data, i, len(diff_data) - 1),
                        y_threshold,
                    )
                peaks.append(peak)
...
def limitProeminences(data, peaksPos, threshold):
    ...
    for i in range(1, len(peaksPos) - 1):
        proeminence = calculateProeminencePeaks(
            getValuesArray(data, peaksPos[aux], peaksPos[i + 1]),
            peaksPos[i] - peaksPos[aux],
        )
...
def correctPeaks(data, peaksPos):
    ...
    for i in range(len(peaksPos)):
        if i == 0:
            ...
            peakPos, peak, valleyPos, valley = getCorrectPeak(
                new_data, peaksPos[i] - peaksPos[i - 1]
            )
        ...
        valleys[i] = valley
    return (
        [int(v) for v in new_peaksPos],
        new_peaks,
        [int(v) for v in valleysPos],
        valleys,
    )
...
def calculateProeminencePeaks(data, peakPos):
    first_proeminence = (
        data[int(peakPos)] - minimumValue(getValuesArray(data, 0, peakPos))[1]
    )
    second_proeminence = (
        data[int(peakPos)] - minimumValue(getValuesArray(data, peakPos, len(data)))[1]
    )
...
def calculateProeminencePeaks(data, threshold):
    ...
    for i in range(1, len(data) - 1):
        ...
        if (
            data[i - 1] < data[i]
            and data[i] > data[i + 1]
            and data[i] - min_value > threshold
        ):
            peaksPos.append(i)
            peaks.append(data[i])
            max_value = min_abs_value
...
def detect_SpO2Peaks(red, infrared, fs):
    ...
    filt_red = bsnb.bandpass(red, 0.5, 5, 2, fs, True)
    filt_infrared = bsnb.bandpass(infrared, 0.5, 5, 2, fs, True)
    ...
    if len(aux) > 1 * fs:
        ...
        baseline = mean(aux)
    signal = aux - baseline
    aux = bsnbSmooth(signal, int(0.3 * fs))
    # aux = bsnb.lowpass(signal, 3, 2,  fs, True)

    peak, peakPos, valley, valleyPos = peakDetection(
        aux, proeminence=0.9 * np.std(aux), x_threshold=0.3 * fs
    )
