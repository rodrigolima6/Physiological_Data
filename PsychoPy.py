    ...
    for users in sorted(data.keys()):
        ...
        for index, row in data[users].iterrows():
            if "Horror" in row["videos"]:
                d = {
                    "videos": row["videos"],
                    "valence": row["slider_valence.response"],
                    "arousal": row["slider_arousal.response"],
                    "dominance": row["slider_dominance.response"],
                    "anticipatory": row["anticipatory_slider.response"],
                    "perceived_control": row["perceived_obstacles_slider.response"],
                    "novelty": row["novelty.response"],
                    "unexpectedness": row["unexpectedness.response"],
                    "intrisic_goal": row["intrisic_goal.response"],
...
def Full_Dataframe(data, dimension):
    ...
    df = (((df_erotic.join(df_horror)).join(df_negative)).join(df_scenery)).join(
        df_positive
    )
