...
class PerlinNoise(object):
    ...
    def noise(self, coords):
        ...
        res = (
            self.drop(abs(uvw)).prod(axis=2) * prod([gradiens, uvw], axis=0).sum(axis=2)
        ).sum(axis=1)

        res[res > 1.0] = 1.0
        res[res < -1.0] = -1.0

        return ((res + 1) * 128).astype(float)
    ...
    def __init__(self, size=None, n=None):
        ...
        self.idx_ar = (
            indices(2 * ones(self.order), dtype=int8).reshape(self.order, -1).T
        )
